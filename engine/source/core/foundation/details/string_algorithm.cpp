...
//    }
    ...
    strsize UBMFindInsensitive(const char16_t* haystack, strsize alen, strsize from, const char16_t* needle, strsize blen, const char16_t* skipTable)
    {
        ...
        while (current < end)
        {
            size_t skip = skipTable[Utf16::FoldCase(current, start) & 0xff];
            if (!skip)
            {
                while (skip < blen)
                {
                    if (Utf16::FoldCase((current - skip), start) != Utf16::FoldCase((needle + plMinusOne - skip), start))
                    {
                        break;
                    }
                    ++skip;
                ...
                if (skipTable[Utf16::FoldCase((current - skip), start) & 0xff] == blen)
                {
                    skip = blen - skip;
                }
                else
    ...
    static int32 UFindStringBoyerMooreInsensitive(const char16_t* haystack, strsize alen, strsize from, const char16_t* needle, strsize blen)
    {
        char16_t skipTable[256];
        UBMInitSkipTableInsensitive(needle, blen, skipTable);
        return UBMFindInsensitive(haystack, alen, from, needle, blen, skipTable);
    }
    ...
    strsize UFindStringInsensitive(const char16_t* haystack, strsize alen, strsize from, const char16_t* needle, strsize blen)
    {
        ...
        if (blen == 1)
        {
            return FindChar(haystack, alen, from, *needle);
        }

        if (alen > 500 && blen > 5)
        {
            return UFindStringBoyerMooreInsensitive(haystack, alen, from, needle, blen);
        }
        ...
        {
            hashNeedle = (hashNeedle<<1) + UChar::FoldCase(needle + idx, needle);
            hashHaystack = (hashHaystack<<1) + UChar::FoldCase(current + idx, haystackStart);
        }
        hashHaystack -= UChar::FoldCase(current + slMinusOne, haystackStart);

        while (current <= end)
        {
            hashHaystack += UChar::FoldCase(current + slMinusOne, haystackStart);
            if (hashHaystack == hashNeedle && CharUtils::UCompareInsensitive(needle, current, blen) == 0)
            {
                return (strsize)(current - haystack);
            }

            REHASH(UChar::FoldCase(current, haystackStart));
            ++current;
        }
        return -1;
    }
